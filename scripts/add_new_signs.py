...
NEW: dict[str, tuple[str, str, str, list[str]]] = {
    ...
    "B15": (
        ...
        "B15 : rond à flèches, vous devez laisser passer le sens opposé dans le rétrécissement. Voisin du AB5 (carré).",
    ...
    "C26b": (
        "indication",
        "Fin de voie de détresse",
        "C26b : fin de la voie d'arrêt d'urgence en descente (C26a).",
        ["France road sign C26b.svg"],
    ),
    "C62": (
        "indication",
        "Présignalisation d'une issue de secours",
        "C62 : issue de secours, surtout en tunnel. La connaître avant d'en avoir besoin.",
        ["France road sign C62.svg"],
    ),
    "C64a": (
        "indication",
        "Issue de secours",
        "C64a : localise une issue de secours. En tunnel, descendre moto arrêtée, suivre le pictogramme.",
    ...
    "CE1": (
        ...
        "CE1 : station-service. Utile pour anticiper la réserve.",
        ["France road sign CE1.svg"],
    ),
    "CE2a": (
        "service",
        "Restaurant",
        "CE2a : restaurant à proximité.",
        ["France road sign CE2a.svg"],
    ),
    "CE3a": (
        "service",
        "Hôtel ou motel",
        "CE3a : hébergement à proximité.",
        ["France road sign CE3a.svg"],
    ),
    "CE4a": (
        "service",
        "Point d'information",
        "CE4a : office de tourisme / point info.",
        ["France road sign CE4a.svg"],
    ),
    "CE12": (
        "service",
        "Poste de secours / hôpital",
        "CE12 : soins d'urgence à proximité. En accident, 112 reste le réflexe.",
    ...
    "CE27": (
        ...
        "CE27 : recharge de véhicules électriques.",
    ...
    "M4a": (
        ...
        "M4a : le panneau principal s'applique aux motocyclettes. Crucial : un B14 + M4a limite seulement les motos.",
        ["France road sign M4a.svg"],
    ),
    "M4b": (
        "panonceau",
        "Panonceau « cycles »",
        "M4b : s'applique aux cycles, pas aux motos.",
        ["France road sign M4b.svg"],
    ),
    "M4c": (
        "panonceau",
        "Panonceau « véhicules de transport de marchandises »",
        "M4c : s'applique aux marchandises / PL, pas à une moto solo.",
    ...
    "M5": (
        "panonceau",
        "Panonceau de catégorie d'usagers (pictogramme)",
        "M5 : précise à qui s'applique le panneau (souvent un usager vulnérable).",
        ["France road sign M5.svg"],
    ),
    "M6a": (
        "panonceau",
        "Panonceau « sauf… »",
        "M6a : exception à l'interdiction (riverains, services…). Lire avant de s'engager.",
        ["France road sign M6a.svg"],
    ),
    "M9d": (
        "panonceau",
        "Panonceau « rappel »",
        "M9d : rappelle une règle déjà en vigueur, sans en créer une nouvelle.",
...
def patch_questions(available: list[str]) -> None:
    ...
    extra_behavior = [
        ...
        },
        {
            ...
            "choices": [
                {"id": "a", "text": "Aux motocyclettes uniquement"},
                {"id": "b", "text": "À tous les véhicules"},
                {"id": "c", "text": "Aux cycles seulement"},
                {"id": "d", "text": "Aux poids lourds"},
            ],
            "correct": ["a"],
            "explanation": "M4a = pictogramme moto. Le panneau principal ne vise alors que les motocyclettes.",
            "multi": False,
            "image": "assets/img/signs/M4a.svg",
            "imageAlt": "Panonceau motocyclettes",
            "imageCredit": CREDIT,
        },
        {
            "id": "sig-b25-1",
